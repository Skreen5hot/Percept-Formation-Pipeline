...
// === CLEAN: all 9 roles -> witnessed-identity nodes; ProjectionRecord present ===
{
  ...
  ok(objOf(tt, F, 'fan:hasCustomer')[0] === 'fdata:Customer/C1'
    && typesOf(tt, 'fdata:Customer/C1').includes('fan:Customer'), 'clean: resolved hasCustomer -> witnessed-identity fdata:Customer/C1 typed fan:Customer');
  ok(objOf(tt, F, 'fan:hasEmployee')[0] === 'fdata:Employee/E1', 'clean: resolved ACCIDENTAL role materializes too (no silent loss)');
...
// === ICE (missing order_date): absent constitutive -> ICE, NEVER typed as concernsType, carries fsdd:role ===
{
  const { triples: tt } = project(STAR_SAMPLES.ice.factRows);
  const ice = objOf(tt, 'fdata:ActOfOrdering/ord-2', 'fan:orderOccupies')[0];
  ok(ice && ice.startsWith('fdata:ice/') && typesOf(tt, ice).includes('fsdd:ImplicitEntityRecord'), 'ice: absent orderOccupies -> ImplicitEntityRecord node');
  ...
  ok(objOf(tt, ice, 'fsdd:role')[0] === 'orderOccupies', 'ice: the ICE carries its structured fsdd:role (sec 3a.1)');
}

// === ACCIDENTAL-BROKEN (shipper dangling): UnresolvedRole, NEVER typed, NEVER dropped, frame survives ===
{
  const { triples: tt } = project(ACCIDENTAL);
  const u = objOf(tt, 'fdata:ActOfOrdering/ord-5', 'fan:hasShipper')[0];
  ok(u && u.startsWith('fdata:unresolved/') && typesOf(tt, u).includes('fsdd:UnresolvedRole'), 'accidental: broken hasShipper -> UnresolvedRole node (present, not dropped)');
  ok(!typesOf(tt, u).includes('fan:Shipper') && objOf(tt, u, 'fsdd:reason')[0] === 'broken-ref', 'accidental: NEGATIVE -- not typed fan:Shipper; names reason broken-ref');
...
// === EXCLUDED (orphan customer): NO valid frame; reason names every constitutive dangler ===
{
  ...
  ok(x && objOf(tt, x, 'fsdd:reason')[0] === 'dangling-constitutive:hasCustomer', 'orphan: ExcludedFrame reason names hasCustomer');
}
{
  const { triples: tt } = project(DOUBLE);
  const x = tt.filter((t) => t.p === 'rdf:type' && t.o === 'fsdd:ExcludedFrame').map((t) => t.s)[0];
  const reason = objOf(tt, x, 'fsdd:reason')[0] || '';
  ok(reason.includes('hasCustomer') && reason.includes('hasProduct'), 'double: ExcludedFrame reason names BOTH constitutive danglers (sec 3a.2 end-to-end)');
}

// === WITNESSED COREFERENCE: two orders by C1 -> the SAME node ===
{
  const { triples: tt } = project([CLEAN, { ...CLEAN, order_id: 9 }]);
  const c1 = objOf(tt, 'fdata:ActOfOrdering/ord-1', 'fan:hasCustomer')[0];
  const c9 = objOf(tt, 'fdata:ActOfOrdering/ord-9', 'fan:hasCustomer')[0];
  ok(c1 === 'fdata:Customer/C1' && c9 === 'fdata:Customer/C1', 'coreference: two orders by C1 reference the SAME witnessed node');
}
...
// groundedConcept, never the mapping fallback. Real data coincides the two, so only synthetic divergence tests it. ===
{
  ...
  const cust = out.triples.find((t) => t.p === 'fan:hasCustomer').o;
